...
var prefire_start = '2020-07-05';   
var prefire_end = '2020-07-10';

// Now set the same parameters for AFTER the fire.
var postfire_start = '2020-08-18';
var postfire_end = '2020-08-23';

// Set timeframe for structural change analysis
var prefire_notreat1 = '2019-07-05';
var prefire_notreat2 = '2019-07-10';

var prefire_treat1 = '2020-07-05';
var prefire_treat2 = '2020-07-10';
...
//And imagery from before and after treatment  
var pretreatImCol = ee.ImageCollection(imagery
    // Filter by dates.
    .filterDate(prefire_notreat1, prefire_notreat2)
    // Filter by location.
    .filterBounds(area));
...
//And images from after treatment but before fire
var posttreatImCol = ee.ImageCollection(imagery
    // Filter by dates.
    .filterDate(prefire_treat1, prefire_treat2)
    // Filter by location.
    .filterBounds(area));
...
print("Pre-treatment Image Collection: ", pretreatImCol); 
print("Post-treatment Image Collection: ", posttreatImCol);
...
if (platform == 'S2' | platform == 's2') {
  ...
  var pretreat_CM_ImCol = pretreatImCol.map(maskS2sr);
  var posttreat_CM_ImCol = posttreatImCol.map(maskS2sr);
} else {
  var prefire_CM_ImCol = prefireImCol.map(maskL8sr);
  var postfire_CM_ImCol = postfireImCol.map(maskL8sr);
  var pretreat_CM_ImCol = pretreatImCol.map(maskL8sr);
  var posttreat_CM_ImCol = posttreatImCol.map(maskL8sr);
}
...
var pre_tmos = pretreatImCol.mosaic().clip(area);
var post_tmos = posttreatImCol.mosaic().clip(area);

var pre_tcm_mos = pretreat_CM_ImCol.mosaic().clip(area);
var post_tcm_mos = posttreat_CM_ImCol.mosaic().clip(area);

// Add the clipped images to the console on the right
print("Pre-fire True Color Image: ", pre_mos); 
print("Post-fire True Color Image: ", post_mos);
print("Pre-treat True Color Image: ", pre_mos); 
print("Post-treat True Color Image: ", post_mos);
...
// And NBR for change due to treatment
if (platform == 'S2' | platform == 's2') {
  var pretreat = pre_tcm_mos.normalizedDifference(['B8', 'B12']);
  var posttreat = post_tcm_mos.normalizedDifference(['B8', 'B12']);
} else {
  var pretreat = pre_tcm_mos.normalizedDifference(['B5', 'B7']);
  var posttreat = post_tcm_mos.normalizedDifference(['B5', 'B7']);
}


// Add the NBR and treatment change images to the console on the right
//print("Pre-fire Normalized Burn Ratio: ", preNBR); 
//print("Post-fire Normalized Burn Ratio: ", postNBR);

//----------------- Calculate NDVI for pre-fire images
if (platform == 'S2' | platform == 's2') {
  var preNDVI = pre_cm_mos.normalizedDifference(['B8', 'B4']);
} else {
  var preNDVI = pre_cm_mos.normalizedDifference(['B5', 'B4']);
}
...
var dNBR_treat_unscaled = pretreat.subtract(postNBR);

// Scale product to USGS standards
var dNBR = dNBR_unscaled.multiply(1000);
var dNBR_treat = dNBR_treat_unscaled.multiply(1000);

// Add the difference image to the console on the right
print("Difference Normalized Burn Ratio: ", dNBR);
print("Difference Normalized Burn Ratio due to treatment: ", dNBR_treat);

// Add NDVI to the console on the right
print("Normalized Differential Vegetation Index: ", preNDVI);
...
//--NDVI--
Map.addLayer(preNDVI, {min: -1000, max: 1000, palette: grey}, 'NDVI greyscale')
...
Map.addLayer(dNBR_treat.sldStyle(sld_intervals), {}, 'dNBR Treatment classified');

// Seperate result into 8 burn severity classes
var thresholds = ee.Image([-1000, -251, -101, 99, 269, 439, 659, 2000]);
var classified = dNBR.lt(thresholds).reduce('sum').toInt();
var classified = dNBR_treat.lt(thresholds).reduce('sum').toInt();
...
print('Treatment Area by Treatment Class', arealist, '--> click list objects for individual classes');
